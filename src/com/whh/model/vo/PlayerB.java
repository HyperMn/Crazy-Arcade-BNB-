...
public class PlayerB extends SuperElement{
	...
	public PlayerB(int x,int y,int w,int z,ImageIcon img){
		super(x,y,w,z);
		this.img=img;//就近原则
		speed=0;
		bubblenum=1;
	...
	public static PlayerB createPlayerB(String str){
//		playerA,bubble,300,150,40,40
		...
		return new PlayerB(x,y,w,h,img);
	}
	...
	public void addBubble() {
		...
		if(addbubnum<bubblenum&&bubblelist.get((getX()+23)/40+(getY()+53)/40*15)!=1) {
			...
			list.add(Bubble.createBubble(getX()+23, getY()+53, "img/bubble/bubble1.png", "twoplayer",power));
			addbubnum++;
		}
	}
	public void move(){
	...
	public void burst() {
		if(isVisible()) {
			...
			if(condition==0) {
				bursttime++;
				if(bursttime==50||ElementManager.getManager().getElementList("player").size()>0&&(ElementManager.getManager().getElementList("player").get(0).getLayer()==getLayer()&&ElementManager.getManager().getElementList("player").get(0).getX()==getX())) {
					setVisible(false);
				}
			}
		}
